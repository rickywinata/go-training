...
var ProductWhere = struct {
	...
	Price:     whereHelpernull_Int{field: "\"product\".\"price\""},
...
var (
	productAllColumns            = []string{"name", "price", "created_at", "updated_at", "deleted_at"}
	productColumnsWithoutDefault = []string{"name", "price", "created_at", "updated_at", "deleted_at"}
	productColumnsWithDefault    = []string{}
	productPrimaryKeyColumns     = []string{"name"}
)
...
func Products(mods ...qm.QueryMod) productQuery {
	mods = append(mods, qm.From("\"product\""), qmhelper.WhereIsNull("\"product\".\"deleted_at\""))
	return productQuery{NewQuery(mods...)}
}

// FindProduct retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindProduct(ctx context.Context, exec boil.ContextExecutor, name string, selectCols ...string) (*Product, error) {
	productObj := &Product{}
	...
	query := fmt.Sprintf(
		"select %s from \"product\" where \"name\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Product) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = "DELETE FROM \"product\" WHERE \"name\"=$1"
	} else {
		currTime := time.Now().In(boil.GetLocation())
		o.DeletedAt = null.TimeFrom(currTime)
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"product\" SET %s WHERE \"name\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o ProductSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"product\" SET %s WHERE "+
...
func (o *Product) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindProduct(ctx, exec, o.Name)
	if err != nil {
		return err
	}
...
func ProductExists(ctx context.Context, exec boil.ContextExecutor, name string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"product\" where \"name\"=$1 and \"deleted_at\" is null limit 1)"
