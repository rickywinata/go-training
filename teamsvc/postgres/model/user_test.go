...
func testUserToManyOwnerUserTeams(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.OwnerUserTeams().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadOwnerUserTeams(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerUserTeams); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OwnerUserTeams = nil
	if err = a.L.LoadOwnerUserTeams(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerUserTeams); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyMemberUserTeamMembers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.MemberUserTeamMembers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if v.MemberUserID == b.MemberUserID {
			bFound = true
		}
		if v.MemberUserID == c.MemberUserID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadMemberUserTeamMembers(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MemberUserTeamMembers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.MemberUserTeamMembers = nil
	if err = a.L.LoadMemberUserTeamMembers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.MemberUserTeamMembers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpOwnerUserTeams(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddOwnerUserTeams(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.OwnerUserTeams[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OwnerUserTeams[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OwnerUserTeams().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpMemberUserTeamMembers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddMemberUserTeamMembers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.MemberUserID {
			t.Error("foreign key was wrong value", a.ID, first.MemberUserID)
		}
		if a.ID != second.MemberUserID {
			t.Error("foreign key was wrong value", a.ID, second.MemberUserID)
		}

		if first.R.MemberUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.MemberUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.MemberUserTeamMembers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.MemberUserTeamMembers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.MemberUserTeamMembers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Name`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
