...
var UserRels = struct {
	OwnerUserTeams        string
	MemberUserTeamMembers string
}{
	OwnerUserTeams:        "OwnerUserTeams",
	MemberUserTeamMembers: "MemberUserTeamMembers",
}

// userR is where relationships are stored.
type userR struct {
	OwnerUserTeams        TeamSlice       `boil:"OwnerUserTeams" json:"OwnerUserTeams" toml:"OwnerUserTeams" yaml:"OwnerUserTeams"`
	MemberUserTeamMembers TeamMemberSlice `boil:"MemberUserTeamMembers" json:"MemberUserTeamMembers" toml:"MemberUserTeamMembers" yaml:"MemberUserTeamMembers"`
}
...
var (
	...
	userColumnsWithoutDefault = []string{"id", "name", "created_at", "updated_at", "deleted_at"}
...
// OwnerUserTeams retrieves all the team's Teams with an executor via owner_user_id column.
func (o *User) OwnerUserTeams(mods ...qm.QueryMod) teamQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"team\".\"owner_user_id\"=?", o.ID),
...
// MemberUserTeamMembers retrieves all the team_member's TeamMembers with an executor via member_user_id column.
func (o *User) MemberUserTeamMembers(mods ...qm.QueryMod) teamMemberQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"team_member\".\"member_user_id\"=?", o.ID),
...
// LoadOwnerUserTeams allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadOwnerUserTeams(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`team`),
		qm.WhereIn(`team.owner_user_id in ?`, args...),
		qmhelper.WhereIsNull(`team.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	...
	if singular {
		object.R.OwnerUserTeams = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OwnerUserID {
				local.R.OwnerUserTeams = append(local.R.OwnerUserTeams, foreign)
				if foreign.R == nil {
					foreign.R = &teamR{}
				}
...
// LoadMemberUserTeamMembers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadMemberUserTeamMembers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`team_member`),
		qm.WhereIn(`team_member.member_user_id in ?`, args...),
		qmhelper.WhereIsNull(`team_member.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	...
	if singular {
		object.R.MemberUserTeamMembers = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.MemberUserID {
				local.R.MemberUserTeamMembers = append(local.R.MemberUserTeamMembers, foreign)
				if foreign.R == nil {
					foreign.R = &teamMemberR{}
				}
...
// AddOwnerUserTeams adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.OwnerUserTeams.
// Sets related.R.OwnerUser appropriately.
func (o *User) AddOwnerUserTeams(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Team) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.ID, rel.OwnerUserID}
	...
	if o.R == nil {
		o.R = &userR{
			OwnerUserTeams: related,
		}
	} else {
		o.R.OwnerUserTeams = append(o.R.OwnerUserTeams, related...)
	}
...
// AddMemberUserTeamMembers adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.MemberUserTeamMembers.
// Sets related.R.MemberUser appropriately.
func (o *User) AddMemberUserTeamMembers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TeamMember) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.MemberUserID = o.ID
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
	...
	if o.R == nil {
		o.R = &userR{
			MemberUserTeamMembers: related,
		}
	} else {
		o.R.MemberUserTeamMembers = append(o.R.MemberUserTeamMembers, related...)
	}
