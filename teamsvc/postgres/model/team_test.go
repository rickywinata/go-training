...
func testTeamsExists(t *testing.T) {
	...
	e, err := TeamExists(ctx, tx, o.ID, o.OwnerUserID)
	if err != nil {
...
func testTeamsFind(t *testing.T) {
	...
	teamFound, err := FindTeam(ctx, tx, o.ID, o.OwnerUserID)
	if err != nil {
		t.Error(err)
	}
...
func testTeamToManyTeamMembers(t *testing.T) {
	...
	check, err := a.TeamMembers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
...
func testTeamToOneUserUsingOwnerUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOwnerUser(ctx, tx, false, (*[]*Team)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTeamToOneSetOpUserUsingOwnerUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		...
		if exists, err := TeamExists(ctx, tx, a.ID, a.OwnerUserID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	teamDBTypes = map[string]string{`ID`: `uuid`, `OwnerUserID`: `uuid`, `Name`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
